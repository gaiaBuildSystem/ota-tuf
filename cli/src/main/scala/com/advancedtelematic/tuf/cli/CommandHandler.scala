...
object CommandHandler {
  ...
  private def buildClientTarget(
    ...
    for {
      ...
      newTarget = {
        val custom =
          TargetCustom(name, version, hardwareIds, format.some, uri, cliUploaded = cliUploaded.some)
  ...
  def handle[S <: TufServerClient](tufRepo: => TufRepo[S],
                                   ...
                                   config: Config)(implicit ec: ExecutionContext): Future[Unit] =
    config.command match {
      ...
      case InitRepo =>
        RepoManagement
          .initialize(
            ...
            config.reposerverUrl
          )
          .map(_ =>
            log.info(
              s"Finished init for ${config.repoName.valueOrConfigError.value} using ${config.credentialsPath}"
            )
          )
        ...
        repoServer.flatMap { client =>
          tufRepo
            ...
            .recover { case _: java.util.NoSuchElementException =>
              log.warn(s"root keys moved offline, but you'll have to manually sign and push key!")
            }
        }

      case GetTargets =>
        repoServer.flatMap { client =>
          tufRepo.pullTargets(client).map { targetsResponse =>
            log.info(targetsResponse.asJson.spaces2)
          }
        }

      case GetUnsignedTargets =>
        tufRepo.canonicalTargets
          .map(_.getBytes)
      ...
      case InitTargets =>
        tufRepo
          .initTargets(
            config.version.valueOrConfigError,
            config.expireOn.getOrElse(Instant.now().plus(DEFAULT_TARGET_LIFETIME))
          )
      ...
      case UploadTarget =>
        for {
          ...
          client <- repoServer
          _ <- tufRepo.uploadTarget(
            client,
            filename,
            config.inputPath.valueOrConfigError,
            Duration(config.timeout, TimeUnit.SECONDS)
          )
        } yield log.info(
          "Target uploaded. You can now add this target to your targets with `targets add`"
        )
      ...
      case GenRepoKeys =>
        assert(
          config.rootKey.isDefined,
          "The base filename for keys must be defined when generating a new key."
        )
        tufRepo.genKeys(config.rootKey.get, config.keyType, config.keySize).map(_ => ())

      case GetUnsignedRoot =>
        tufRepo.canonicalRoot
          .map(_.getBytes)
          .map(config.outputPath.streamOrStdout.write)

      case PullRoot =>
        repoServer
          .flatMap(client => tufRepo.pullRoot(client, config.force))
          .map(_ => log.info("Pulled root.json"))

      case PushRoot =>
        repoServer
          .flatMap(client => tufRepo.pushRoot(client))
          .map(_ => log.info("Pushed root.json"))

      case SignRoot =>
        tufRepo
          .signRoot(config.keyNames, expirationDate(config), config.oldRootKey, config.signatures)
          .map(p => log.info(s"signed root.json saved to $p"))

      case AddRootKey =>
        tufRepo
          .addRoleKeys(RoleType.ROOT, config.keyNames)
          .map(p => log.info(s"keys added to unsigned root.json saved to $p"))

      case RemoveRootKey =>
        tufRepo
          .keyIdsByName(config.keyNames)
          .flatMap { keyIds =>
      ...
      case AddTargetsKey =>
        tufRepo
          .addRoleKeys(RoleType.TARGETS, config.keyNames)
      ...
      case RemoveTargetsKey =>
        tufRepo
          ...
          .map { case (p, c) =>
            log.info(s"$c keys removed from unsigned targets.json saved to $p")
          }
      ...
      case SignDelegation =>
        config.keyNames
          .map(userKeyStorage.readKeyPair)
          .sequence
          .flatMap { keyPairs =>
            Delegations.signPayload(
              keyPairs,
              config.inputPath.valueOrConfigError,
              WriteOutput.fromConfig(config)
            )
          }
      ...
      case AddDelegationToTarget =>
        config.keyPaths
          .map(CliKeyStorage.readPublicKey)
          .sequence
          .flatMap { keys =>
            tufRepo.addTargetDelegation(
              config.delegationName,
              keys,
              config.delegatedPaths,
              threshold = 1
            )
          }
          .map(_ => ())
      ...
      case PullDelegation =>
        delegationsServer.flatMap { server =>
          Delegations
            .pull(server, config.delegationName, WriteOutput.fromConfig(config))
            .map(_ => ())
        }
